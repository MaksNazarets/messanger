import {isMessageVisible} from './custom_functions.js'
...
const to_last_msg_btn = right_panel.querySelector('.to-last-msg-btn')
...
socket.on('new_message', (msg_data) => {
    if (msg_data['sender_id'] != open_chat_companion_id
        ...
        if (msg_data['sender_id'] == open_chat_companion_id || msg_data['my-msg']) {
            let last_msg = chat_scrollable.querySelector('.message');
...
function populate_with_chat_items(chat_users, parentEl) {
    ...
    let chat_items = parentEl.querySelectorAll('.chat-item') || []
